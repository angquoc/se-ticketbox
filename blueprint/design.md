...
## 9. Thiết kế các Module còn thiếu

### 9.1. Notification Module

Notification Module gửi email và thông báo bất đồng bộ qua BullMQ.
...
* **EmailService** (`modules/notification/services/email.service.ts`): dùng Nodemailer, gửi email xác nhận đơn hàng, email thông báo hết hạn.
* **NotificationProcessor** (`worker/processors/notification.processor.ts`): BullMQ consumer, xử lý job `send-order-paid-email`.
* Cấu hình SMTP qua biến môi trường: `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_SECURE`, `EMAIL_USER`, `EMAIL_PASS`, `EMAIL_FROM`.
...
EMAIL_FROM="TicketBox" <noreply@ticketbox.com>
```

Email là notification channel mặc định. Trong tương trung có thể bổ sung SMS, Zalo OA hoặc push notification bằng cách thêm Notification Provider mới.

---

### 9.2. Ticket Module

Ticket Module cho phép khán giả xem danh sách vé đã mua và lấy mã QR để soát vé.

#### Các endpoint

| Method | Path | Quyền | Mô tả |
|--------|------|--------|--------|
| GET | `/tickets/me` | CUSTOMER | Danh sách vé đã mua (phân trang) |
| GET | `/tickets/me/concerts/:concertId` | CUSTOMER | Vé của một concert cụ thể |
| GET | `/tickets/:id` | CUSTOMER (owner) | Chi tiết một vé |
| GET | `/tickets/:id/qr` | CUSTOMER (owner) | Lấy mã QR token |

#### QR Token

Mỗi khi thanh toán thành công, hệ thống tạo `Ticket` với `qrTokenHash = SHA256(randomUUID)`. Khi staff quét QR, payload chứa `qrTokenHash` gốc (đã được ký bằng HMAC). Backend verify chữ ký để xác nhận vé hợp lệ.

#### Thành phần

* **TicketService** (`modules/ticket/ticket.service.ts`): truy vấn vé, kiểm tra ownership, trả QR token.
* **TicketController** (`modules/ticket/ticket.controller.ts`): các endpoint REST.
* **TicketModule** (`modules/ticket/ticket.module.ts`).

---

### 9.3. Check-in Module

Check-in Module cho phép nhân sự soát vé quét QR tại cổng.

#### Luồng online (khi có mạng)

1. Staff quét QR trên e-ticket của khán giả.
2. PWA gửi `POST /checkin/scan` kèm JWT staff.
3. Backend kiểm tra JWT, tra ticket trong PostgreSQL, kiểm tra trạng thái.
4. Nếu hợp lệ, ghi `CheckinLog` trạng thái `SUCCESS`, cập nhật `Ticket.status = CHECKED_IN`.
5. PWA hiển thị kết quả cho staff.

#### Luồng offline (khi mất mạng)

1. PWA phát hiện mất mạng, chuyển sang chế độ offline.
2. PWA verify chữ ký QR bằng khóa public đã đồng bộ trước.
3. Nếu QR hợp lệ, PWA ghi log vào IndexedDB với `isOffline = true`.
4. Khi có mạng trở lại, PWA gửi batch log lên `POST /checkin/sync`.
5. Backend xử lý từng record: nếu vé chưa check-in → ghi nhận; nếu đã check-in ở nơi khác → đánh dấu conflict.

#### Các endpoint

| Method | Path | Quyền | Mô tả |
|--------|------|--------|--------|
| POST | `/checkin/scan` | STAFF, ADMIN | Quét một QR |
| POST | `/checkin/sync` | STAFF, ADMIN | Đồng bộ batch log offline |

#### Thành phần (chưa triển khai)

* **CheckinService**: xử lý quét online, sync offline, phát hiện conflict.
* **CheckinController**: endpoint REST.
* **CheckinModule**: module NestJS.

---

### 9.4. Guest List Module

Guest List Module cho phép Organizer nhập danh sách khách mời VIP từ file CSV.

#### Luồng nhập CSV

1. Organizer upload file CSV lên Backend.
2. Backend lưu file vào Object Storage.
3. Backend tạo record `UploadedFile` với `purpose = "GUEST_LIST_CSV"`, `status = "PENDING"`.
4. Backend đẩy job import CSV vào BullMQ.
5. Worker đọc CSV theo batch, validate từng dòng.
6. Dòng hợp lệ được insert/update vào `GuestListEntry`.
7. Dòng lỗi được ghi vào import report.

#### Các endpoint

| Method | Path | Quyền | Mô tả |
|--------|------|--------|--------|
| POST | `/admin/concerts/:concertId/guest-list/upload` | ORGANIZER, ADMIN | Upload file CSV |
| GET | `/admin/concerts/:concertId/guest-list` | ORGANIZER, ADMIN | Xem danh sách khách mời |
| GET | `/admin/concerts/:concertId/guest-list/import-report` | ORGANIZER, ADMIN | Xem báo cáo import |

#### Thành phần (chưa triển khai)

* **GuestListService**: xử lý upload, validate CSV, import batch.
* **GuestListController**: endpoint REST.
* **GuestListModule**: module NestJS.
* **CSVImportProcessor**: BullMQ worker để import CSV bất đồng bộ.

---

## 10. Các sửa đổi kỹ thuật quan trọng

### 10.1. Sửa lỗi Redis user_limit khi thanh toán thành công

**Vấn đề:** Khi user đặt vé, Lua Script `reserve-ticket` tăng `user_limit:{userId}:{ticketTypeId}` trong Redis. Nhưng khi thanh toán thành công, `user_limit` không được giảm xuống. Điều này khiến counter tích lũy sai — user đã thanh toán xong vẫn bị tính là đang giữ vé, dẫn đến sai giới hạn mua ở các lần mua tiếp theo.

**Giải pháp:** Khi thanh toán SUCCESS, sau khi tạo ticket trong PostgreSQL, gọi `RedisService.decrementUserLimit()` để giảm counter `user_limit` xuống, đồng thời xóa `reservation:{orderId}`.

**Hai điểm gọi cần sửa:**

1. `OrderService.finalizePayment()` — dùng cho `/webhooks/payment/:provider`.
2. `PaymentService.markPaymentSuccessAndQueueTicketIssue()` — dùng cho `/payments/webhook/mock`.

### 10.2. Loại bỏ duplicate ticket issuance

**Vấn đề:** `OrderService.finalizePayment()` tạo ticket inline trong transaction PostgreSQL. Đồng thời `PaymentService.markPaymentSuccessAndQueueTicketIssue()` cũng đẩy job `TicketIssueProcessor` vào queue để tạo ticket. Cả hai cùng chạy khi webhook đến → vé bị tạo 2 lần.

**Giải pháp:** Giữ một đường duy nhất:

- `OrderService.finalizePayment()`: tạo ticket inline trong PostgreSQL transaction, sau đó sync Redis và queue notification.
- `PaymentService.markPaymentSuccessAndQueueTicketIssue()`: chỉ cập nhật trạng thái order, sync Redis, và queue notification. **Không** queue `TicketIssueProcessor` vì vé đã được tạo inline.

`TicketIssueProcessor` vẫn giữ lại cho các trường hợp order được đánh dấu PAID qua con đường khác (ví dụ: admin đổi trạng thái thủ công).

### 10.3. Thiết kế QR cho check-in
...
Trong đó `signature = HMAC-SHA256(ticketId:qrTokenHash:timestamp, SECRET_KEY)`.

Khi staff quét QR:

1. Parse payload, tách các thành phần.
2. Verify chữ ký `signature`.
3. Check timestamp không quá 24 giờ.
4. Tra `qrTokenHash` trong PostgreSQL để xác nhận vé tồn tại.
5. Kiểm tra trạng thái ticket: nếu đã `CHECKED_IN` → từ chối; nếu `ISSUED` → check-in thành công.
