...
graph TD
    ...
    subgraph Tester 2: Check-in & Gate
        ...
        TC-T2-02 --> TC-T2-05[TC-T2-05: Double Check-in (Online)]
        TC-T2-03 --> TC-T2-07[TC-T2-07: Double Check-in (Offline)]
        TC-T2-07 -.-> TC-T2-04
    end
...
#### **TC-T2-05**
*   **Requirement**: Ngăn chặn tuyệt đối việc sử dụng 1 vé để check-in 2 lần (Double Check-in) — **kịch bản Online** (thiết bị có mạng).
*   **Tester phụ trách**: Tester 2
*   **Priority**: High
*   **Precondition**: Vé của `customer-t2-03` đã có trạng thái `CHECKED_IN`. Thiết bị của staff đang **trực tuyến** (online).
*   **Seed data**: `concert-checkin-online`
*   **Steps**:
...
*   **Expected**:
    ...
    *   Một bản ghi `CheckinLog` mới được tạo với trạng thái `ALREADY_CHECKED_IN` (`isOffline=false`).
*   **Cleanup sau test**: Không cần dọn dẹp.
*   **Dependency**: `TC-T2-02`
*   **Ghi chú quan trọng**:
    *   Đây là kịch bản cho **nhánh online** (gọi `POST /checkin/verify`) — cơ chế atomic `updateMany({ status: ISSUED })` đã chặn đúng.
    *   Kịch bản **offline** (cùng 1 vé quét 2 lần trong khi không có mạng) xem TC-T2-07.
...
#### **TC-T2-07**
*   **Requirement**: Ngăn chặn tuyệt đối việc sử dụng 1 vé để check-in 2 lần (Double Check-in) — **kịch bản Offline** (thiết bị mất mạng hoàn toàn trong toàn bộ quá trình quét).
*   **Tester phụ trách**: Tester 2
*   **Priority**: **Critical** *(nâng từ High do liên quan đến bảo mật soát vé)*
*   **Precondition**:
    *   Vé của `customer-t2-05` có trạng thái `ISSUED` gán cho `GATE-A`.
    *   Thiết bị của Staff đang **hoàn toàn không có mạng** (Airplane mode bật + không có WiFi).
    *   Ticket ID & QR token của `customer-t2-05` đã được lưu trong thiết bị hoặc có sẵn để quét lặp lại.
*   **Seed data**: `concert-checkin-offline`
*   **Lý do thêm test case**:
    *   Đã phát hiện bug tại thời điểm viết plan: `useOfflineCheckin.ts` tạo `offlineEventId = uuidv4()` cho MỖI lần quét mà KHÔNG query IDB để kiểm tra `ticketId` đã được quét trên cùng thiết bị trong phiên offline hay chưa.
    *   Trong nhánh offline (file `src/frontend/checkin-pwa/src/hooks/useOfflineCheckin.ts:211-246`), hàm `handleScan` trả `status: 'valid'` một cách mặc định cho mọi lượt quét offline → không phát hiện được vé đã quét trước đó trên cùng thiết bị.
    *   Backend có atomic `updateMany({ status: ISSUED })` chặn đúng khi sync, nhưng lúc đó khán giả đã vào cổng → vi phạm yêu cầu "1 vé 1 lần vào cổng".
*   **Steps**:
    1. Đăng nhập `staff-t2-01@ticketbox.vn`, chọn `GATE-A`.
    2. Bật **Airplane mode** (hoặc ngắt WiFi + tắt 4G) trên thiết bị. Chờ badge đỏ "OFFLINE" hiển thị ở header (khoảng 5–10 giây).
    3. Quét QR của vé `customer-t2-05` — **lần 1**.
    4. Chờ màn hình kết quả hiện ra, bấm "Quét tiếp" (Scan Next).
    5. **Quét lại QR của cùng vé `customer-t2-05`** — **lần 2** (trong vòng 30 giây).
    6. Có thể tiếp tục quét lần 3, lần 4 để xác nhận hành vi ổn định.
    7. Tắt Airplane mode, đợi sync tự động chạy.
    8. Truy cập DB để kiểm tra kết quả.
*   **Expected (kỳ vọng đúng — sau khi fix bug ở mức Frontend)**:
    *   Lần quét 1: hiển thị màn hình xanh "Soát vé thành công ngoại tuyến". Badge vàng "1" hiện ở header (1 bản ghi đang chờ sync).
    *   Lần quét 2 (cùng vé): **PHẢI hiển thị màn hình đỏ "VÉ ĐÃ ĐƯỢC CHECK-IN TRƯỚC ĐÓ"** ngay lập tức, không tạo thêm bản ghi offline trong IDB.
    *   Lần quét 3+: vẫn báo lỗi "VÉ ĐÃ ĐƯỢC CHECK-IN TRƯỚC ĐÓ".
    *   Sau khi sync: DB chỉ có **1** bản ghi `CheckinLog` với `status: SUCCESS` cho `customer-t2-05`, không có bản ghi `REJECTED_CONFLICT` nào (vì frontend đã chặn trước).
    *   Trong `CheckinLog` table: truy vấn
        ```sql
        SELECT id, status, "isOffline", conflict
        FROM "CheckinLog"
        WHERE "ticketId" = '<customer-t2-05-id>'
        ORDER BY "createdAt";
        ```
        phải trả về **đúng 1 row** với `status='SUCCESS'`, `isOffline=true`.
*   **Expected (hiện tại — sẽ FAIL do bug)**:
    *   Lần quét 1: OK (xanh).
    *   Lần quét 2: cũng OK (xanh). Badge vàng "2".
    *   Sau sync: DB có **2 rows**: 1 SUCCESS + 1 REJECTED_CONFLICT.
    *   Frontend ở lần 2 đã lừa staff — khán giả đã vào cổng trong khi offline, mặc dù hệ thống cuối cùng phát hiện conflict trong DB.
*   **Cleanup sau test**:
    *   Với kỳ vọng hiện tại (FAIL): reset vé `customer-t2-05` về `ISSUED`, xóa các bản ghi `CheckinLog` trong DB (`DELETE FROM "CheckinLog" WHERE "ticketId" = '<id>'`).
    *   Với kỳ vọng sau fix: chỉ cần reset vé về `ISSUED`.
*   **Dependency**: `TC-T2-03`
*   **Ghi chú đặc biệt**:
    *   Test này **BẮT BUỘC** phải chạy trên thiết bị thật + HTTPS ngrok/cloudflared (xem Section 7 — *Hướng dẫn test offline*) vì localhost không bắn event `offline` đáng tin cậy.
    *   Sau khi fix, cần tái chạy TC-T2-03 và TC-T2-04 để xác nhận không regress flow offline bình thường.
...
| | Chặn quét vé trùng lặp — kịch bản online (server-side atomic) | `TC-T2-05` | [ ] |
|| | Chặn quét vé trùng lặp — kịch bản offline (local IDB check) | `TC-T2-07` | [ ] |
