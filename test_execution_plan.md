...
## 4. DỮ LIỆU SEED CẦN BỔ SUNG (TEST SEED DATA)

Đề xuất bổ sung tệp seed chuyên dụng `data/seed/seed-test.ts` chứa **14 Concert biệt lập** và các tài khoản kiểm thử cô lập như sau:

### Concerts:
1.  `concert-purchase-momo`: Chuyên dùng kiểm thử luồng mua vé và thanh toán MoMo (Tester 1).
2.  `concert-purchase-vnpay`: Chuyên dùng kiểm thử luồng mua vé và thanh toán VNPAY (Tester 1).
3.  `concert-purchase-limits`: Cấu hình SVIP tối đa 1 vé/user, GA tối đa 3 vé/user để test per-user limit (Tester 1).
4.  `concert-purchase-idempotency`: Cấu hình để test trùng lặp idempotency key (Tester 1).
5.  `concert-checkin-online`: Seed sẵn 50 vé trạng thái `ISSUED` để test soát vé online (Tester 2).
6.  `concert-checkin-offline`: Seed sẵn 50 vé trạng thái `ISSUED` để test soát vé offline & sync (Tester 2).
7.  `concert-checkin-gates`: Phân chia cấu hình `GATE-A` và `GATE-B` để test sai Gate (Tester 2).
8.  `concert-admin-crud`: Concert mẫu ở trạng thái `DRAFT` để test CRUD Admin (Tester 3).
9.  `concert-admin-ai`: Phục vụ kiểm thử tải lên PDF trích xuất Bio bằng AI (Tester 3).
10. `concert-admin-csv`: Phục vụ import Guest List VIP (Tester 3).
11. `concert-nft-cache`: Sử dụng đo lường Caching Redis (Tester 4).
12. `concert-nft-concurrency`: Vé SVIP chỉ seed **đúng 02 vé** để test tranh chấp vé cuối (Tester 4).
13. `concert-nft-cb`: Dùng kiểm thử Circuit Breaker khi thanh toán bị sập (Tester 4).
14. `concert-nft-notif`: Cấu hình thời gian bắt đầu sau đúng 23h59m để test BullMQ gửi email nhắc nhở tự động trước 24 giờ (Tester 4).

### Tài khoản kiểm thử (Password mặc định: `admin123` / `organizer123` / `staff123` / `customer123`):
*   **Admin**: `admin-t3-01@ticketbox.vn`
*   **Organizers**: `organizer-t3-01@ticketbox.vn` (sở hữu các concert crud/ai/csv), `organizer-t3-02@ticketbox.vn` (test kiểm tra sở hữu chéo).
*   **Staffs**: `staff-t2-01@ticketbox.vn`, `staff-t2-02@ticketbox.vn` (soát vé tại các cổng).
*   **Customers**:
    *   Tester 1: `customer-t1-01@example.com` -> `customer-t1-05@example.com`
    *   Tester 2: `customer-t2-01@example.com` -> `customer-t2-05@example.com` (đã được mua sẵn vé điện tử)
    *   Tester 3: `customer-t3-01@example.com`
    *   Tester 4: `customer-t4-01@example.com` -> `customer-t4-20@example.com` (Dùng cho kiểm thử Concurrency & Load)

### API Hỗ trợ Reset Dữ liệu (Dev/Staging):
Đề xuất thêm endpoint `POST /health/reset` ở Backend. Khi nhận request, hệ thống sẽ thực hiện truncate toàn bộ bảng database và chạy lại script `seed-test.ts` trong vòng 3-5 giây để đưa DB về trạng thái sạch ban đầu.

---

## 5. DANH SÁCH CÁC KỊCH BẢN KIỂM THỬ (TEST CASES)
...
---
*Tài liệu này được biên soạn bởi QA Lead & Software Architect.*
